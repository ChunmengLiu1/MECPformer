...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # img = np.asarray(img)   # (281, 500, 3)
        # print('=====img.size:=====', img.size)    # (500, 281)
...
class VOC12ClsDatasetMSF_sal(VOC12ClsDataset):

    def __init__(self, img_name_list_path, voc12_root, inter_transform=None):
        super().__init__(img_name_list_path, voc12_root, transform=None)
        self.inter_transform = inter_transform

    def __getitem__(self, idx):
        name, img, label, sal = super().__getitem__(idx)
        ms_img_list = []
        ms_sal_list = []
        for s in [256, 512, 768]:
            ...
            s_sal = sal.resize(target_size, resample=PIL.Image.CUBIC)
            ms_sal_list.append(s_sal)

        if self.inter_transform:
            for i in range(len(ms_img_list)):
                ms_img_list[i] = self.inter_transform(ms_img_list[i])
            for i in range(len(ms_sal_list)):
                ms_sal_list[i] = self.inter_transform(ms_sal_list[i])
        ...
        msf_sal_list = []
        for i in range(len(ms_sal_list)):
            msf_sal_list.append(ms_sal_list[i])
            msf_sal_list.append(np.flip(ms_sal_list[i], -1).copy())

        return name, msf_img_list, msf_sal_list, label
...
class VOC12AffDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        label_la = np.array(PIL.Image.open(label_la_path))
        label_ha = np.array(PIL.Image.open(label_ha_path))
        ...
        for joint_transform, img_transform, label_transform \
            ...
            if joint_transform:
                img_label = np.concatenate((img, np.expand_dims(label_la, axis=2), np.expand_dims(label_ha, axis=2)), axis=-1)
                img_label = joint_transform(img_label)
                img = img_label[..., :3]
                label_la = img_label[..., 3]
                label_ha = img_label[..., 4]

            if img_transform:
                img = img_transform(img)
            if label_transform:
                label_la = label_transform(label_la)
                label_ha = label_transform(label_ha)

        label = label_la.copy().astype(np.uint8)
        label[label_la == 0] = 255
        label[label_la + label_ha == 0] = 0
