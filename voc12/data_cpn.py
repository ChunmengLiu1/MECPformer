...
from tool.imutils import hide_patch,hide_patch_ss
...
class VOC12ClsDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        parent = img.__class__.__name__

        label = torch.from_numpy(self.label_list[idx])

        if parent != 'Image':
            img1,img2,c,uc = hide_patch_ss(img)   #You can substitute "hide_patch" for grid strategy
            return name,img,label,img1,img2,c,uc
        else:
            return name,img,label


class VOC12ClsDataset_ME(VOC12ImageDataset):

    def __init__(self, img_name_list_path, voc12_root, epoch, transform=None):
        super().__init__(img_name_list_path, voc12_root, transform)
        self.label_list = load_image_label_list_from_npy(self.img_name_list)
        self.epoch = epoch
    ...
    def __getitem__(self, idx):
        ...
        if parent != 'Image':
            if self.epoch % 5 == 0:
                img1, img2, c, uc = hide_patch_ss(img, 400)
            elif self.epoch % 5 == 1:
                img1, img2, c, uc = hide_patch_ss(img, 350)
            elif self.epoch % 5 == 2:
                img1, img2, c, uc = hide_patch_ss(img, 300)
            elif self.epoch % 5 == 3:
                img1, img2, c, uc = hide_patch_ss(img, 250)
            elif self.epoch % 5 == 4:
                img1, img2, c, uc = hide_patch_ss(img, 200)
